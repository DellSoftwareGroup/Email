...
<p>Rows are used to separate blocks of content vertically. In addition to the vertical separation, the
	<kbd>&lt;td&gt;</kbd> tags of
	<code>.row</code> tables use the wrapper class to maintain a gutter between columns. Note: the last
	<code>.wrapper</code> <kbd>&lt;td&gt;</kbd> in a row MUST have a class of
	<code>.last</code> applied to it, even if it's the only wrapper in the row (ex. for a row with a single, twelve-column wide content area).
</p>
...
<table>
	...
	<tr>
		...
		<td>wrapper</td>
		<td>Wraps each
			<code>.columns</code> table, in order to create a gutter between columns and force them to expand to full width on small screens.
		</td>
	</tr>
	<tr>
		<td>td</td>
		<td>last</td>
		<td>Class applied to the last
			<code>.wrapper</code> td in each row in order for the gutter to work properly. If you only have one (presumably full-width)
			<code>.columns</code> table (and therefore one <code>.wrapper</code> td) in a row, the
			<code>.wrapper</code> td still needs to have the last class applied to it.
		</td>
	</tr>
	<tr>
		<td>table</td>
		<td>{number}</td>
		<td>Can be any number between one and twelve (spelled out). Used to determine how wide your
			<code>.columns</code> tables are. The number of columns in each row should add up to 12, including <!-- <a href="#offsets"> -->offset columns<!-- </a> -->.
		</td>
	</tr>
	<tr>
		<td>table</td>
		<td>columns</td>
		<td>Table that displays as n-twelfths of the width of the 580px
			<code>.container</code> table on large screens, and expands to the full width of the
			<code>.container</code> table on small screens.
		</td>
	</tr>
	<tr>
		<td>td</td>
		<td>expander</td>
		<td>An empty (and invisible) element added after the content element in a
			<code>.columns</code> table. It forces the content
			<code>td</code> to expand to the full width of the screen on small devices, instead of just the width of the content within the
			<code>td</code>.
		</td>
	</tr>
</table>
...
<p>Ink's 12-column grid makes creating even column layouts a snap. Just use one <code>.twelve.columns</code>, two
	<code>.six.columns</code>, three <code>.four.columns</code> or four
	<code>.three.columns</code> classes to create your layout. Appearing as multiple, even columns on large screens, the layout will fold into a single column on small (mobile) screens.
</p>
...
<p>To center the content of a column, apply a class of <code>center</code> to the
	<kbd>&lt;td&gt;</kbd> that contains the content. If you want to center an image, you should also apply a class of
	<code>center</code> to the image itself. For maximum client support, we also recommend using the HTML
	<kbd>&lt;center&gt;</kbd> tag around the content you wish to center.</p>
...
<p>In lieu of using empty columns to offset content, one can use the
	<code>.offset-by-{number of columns}</code> helper class (ex,<code>.offset-by-two</code>,
	<code>.offset-by-three</code>, etc). By applying the class to the wrapper surrounding a column section, the content will be pushed to the left by the specified number of columns. Remember to count any column offsets towards the total number of columns in a row when making sure that they add up to 12.
</p>
...
<p>A fairly common use case for the container is to give your text a white background to separate it from a colored email body. To assist with this design pattern, we've included three helper classes with Ink:
	<code>.text-pad</code>, <code>.left-text-pad</code> and <code>.right-text-pad</code>.</p>
<p>The <code>.text-pad</code> class provides 10px of padding on either side of the content (while it's named
	<strong>text</strong>-pad, it works just as well on buttons or images), making sure it doesn't butt right up against the edge of your container. The
	<code>.text-pad</code> class should be applied to the <kbd>&lt;td&gt;</kbd> of a <code>.columns</code>
	<kbd>&lt;table&gt;</kbd>.</p>
<p>The <code>.left-text-pad</code> and <code>.right-text-pad</code> do the same thing as the
	<code>.text-pad</code> class, but are meant to be applied to content that doesn't extend across the entire row (in other words, content that isn't twelve columns wide). The directional text-padding classes only apply the padding to the side of the content near the edge of the container on large screens, but apply it to both sides on small screens, when the columns expand to fill the full width of the row.
</p>
...
<p>Visual example of the body and the container having different colored backgrounds, the main use case for
	<code>.text-pad</code>.</p>
...
<p>When the Ink grid is shown on a small screen, the
	<code>.columns</code> tables expand to the full width of the container and stack vertically. On some clients, however, the columns don't expand properly if the content isn't as wide as the screen. While this might not seem so bad, it can cause your layout to appear broken if you are using a background color on the
	<code>.columns</code> table or are centering the content.</p>
<p>To get around this, an empty <kbd>&lt;td&gt;</kbd> with a class of <code>expander</code> is used after the
	<kbd>&lt;td&gt;</kbd> containing the actual content in the <code>.columns</code> table. This extra
	<kbd>&lt;td&gt;</kbd> isn't displayed, but it forces the content <kbd>&lt;td&gt;</kbd> to expand to full width.</p>
<h6>Expander TDs</h6>
<?php code_example(
	'<table class="columns">
  <tr>
...
<div class="compatibility-section">
	<div class="row">
		<div class="large-9 columns">
			<p>The grid works as expected in most major email clients, with some caveats in
				<strong>Gmail (Mobile, iOS, Android)</strong>.</p>
		</div>
		...
		<div class="small-12 columns">
			<table>
				...
				<tr>
					...
					<td>Since Gmail doesn't support media queries, users will be presented with the desktop view. If mobile Gmail is a large part of your audience, we'd suggest using a layout based on the
						<a href="#block-grid">block-grid</a>.
					</td>
				</tr>
				<tr>
				...
				<tr>
					<td>Outlook.com</td>
					<td><span class="check">&#10004;</span></td>
					<td>Be sure to use <kbd>&lt;center&gt;</kbd> tags in addition to the
						<code>center</code> class in order to center content properly.
					</td>
				</tr>
				</tbody>
			</table>
